...
class LoginPage{
    ...
    async verifysuccessfullLogout(){
      ...
      try {
        ...
        await expect(navigate.loginButton).toBeEnabled();

        const isLogoutButtonVisible = await this.logoutButton.isVisible();
        const isProfilButtonVisible = await this.profilButton.isVisible();
        const isLoginButtonEnabled = await navigate.loginButton.isEnabled();

        console.log('Logout button is visible:', isLogoutButtonVisible);
        console.log('Profil button is visible:', isProfilButtonVisible);
        console.log('Login button is enabled:', isLoginButtonEnabled);
